...
let getdata = async(viaID, param) => {
    ...
    await database.ref('audio/'+videoInfo.videoDetails.videoId).once('value').then((snapshot) => {
        let v = snapshot.val()
        console.log(v)
        if(v && v.url) {
    ...
    return new Promise(async function(resolve, reject) {
        await download(audioFormat.url, videoInfo.videoDetails.videoId).then(function(ur) {
  ...
  let download = async(a_url, v_id) => {
    return new Promise(function async(resolve, reject) {
        ...
        let response = await fetch(a_url).catch()
